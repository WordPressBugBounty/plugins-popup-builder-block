    ...
    public function wpmet_apps_renderer()
    {
        ...
        foreach ($this->plugins as $key => $plugin) {
            ...
            if (!empty($plugin_docs)) {
                echo sprintf('<a target="_blank" rel="noopener noreferrer" href="%1$s" class="wpmet-onboard-tut-term--help">%2$s</a>', esc_url($plugin_docs), esc_html__('Read Docs', 'popup-builder-block'));
            }
            ?>
								</div>
							</label>
